...
void initZobrist() {
    int offset = 0;
    //color, enPassant, castleRights hashes
    blackHash = Random64[780];
    for (int i = 0; i < 4; i++) {
        castleHash[i] = Random64[768 + i];
    }
    for (int i = 0; i < 8; i++) {
        enPassantHash[i] = Random64[772 + i];
    }
    //piece hashes
	for (int i = 0; i < 12; i++) {
		for (int j = 0; j < 64; j++) {
            zobristTable[i][j] = Random64[offset];
            offset++;
		}
	}
    //std::cout << offset;
...
void computeZobrist(int board[], int color, uint64_t miscBoards[]) {
    ...
    if (miscBoards[3] & one << 0) {
        zobristKey ^= castleHash[3];
    }
    if (miscBoards[3] & one << 7) {
        zobristKey ^= castleHash[2];
    }
    if (miscBoards[3] & one << 56) {
        zobristKey ^= castleHash[1];
    }
